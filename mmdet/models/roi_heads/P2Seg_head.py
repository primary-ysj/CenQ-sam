...
class P2SegHead(StandardRoIHead):
    ...
    def __init__(self, bbox_roi_extractor, num_stages, bbox_head, top_k=7, with_atten=None, **kwargs):
        ...
        if self.with_atten:
            self.Pts_attention = Pts_attention(256)
    ...
    def _bbox_forward_train(self, x, proposal_list_base, proposals_list, proposals_valid_list, neg_proposal_list,
        ...
        # if stage ==0:
        #     proposals_list,bbox_results = self.filter_box(bbox_results, proposals_list, gt_labels, gt_points, img_metas)
        ...
        #                                                             img_metas)
        # self.show_box(filtered_boxes, filtered_scores, neg_proposal_list, neg_weight_list, bbox_results, gt_points,
        #               img_metas)
    ...
    def _bbox_forward(self, x, rois, gt_points, stage):
        ...
        cls_score, ins_score, reg_box,_ = self.bbox_head(bbox_feats, stage)
    ...
    def _mask_forward_train(self, x, proposals_list, gt_bboxes, gt_labels, dynamic_weight,
        ...
        # mask_atten_map = mask_results['mask_atten_map']
        # mask_atten_map = mask_atten_map.permute(1,0,...)
        # cls_segms_list, im_mask_list = self.mask_head.get_seg_masks(mask_atten_map, proposals_list, gt_labels,
        #                                                             img_metas, rescale=False, threshold=0.0015)
        # for i in range(len(im_mask_list)):
        #     im_mask_list[i]
        #     import cv2
        #     import numpy as np
        #     cv2.namedWindow("ims1", 0)
        #     cv2.namedWindow("ims2", 0)
        #     img=cv2.imread(img_metas[i]['filename'])
        #     cv2.imshow('ims2',img)
        #     cv2.imshow('ims1', np.array(im_mask_list[i][1].float().cpu()))
        #     cv2.waitKey()
    ...
    def _mask_forward(self, x, rois=None, pos_inds=None, bbox_feats=None):
        ...
        mask_cls_pred, mask_ins_pred, atten_map = self.mask_head(mask_feats)
        mask_results = dict(mask_cls_pred=mask_cls_pred, mask_ins_pred=mask_ins_pred, mask_feats=mask_feats,
                            mask_atten_map=atten_map[:, :, 0, 1:].reshape(*atten_map.shape[:2], *mask_feats.shape[-2:]))
        return mask_results
    ...
    def simple_test_pseudo(self, *args, **kwargs):
        """Adapt the legacy ROI head to the point detector contract.

        ``P2Seg.simple_test`` runs several cascade stages and expects each
        ROI head call to return bbox results, pseudo boxes, and proposal
        metadata.  The legacy ``P2SegHead`` returned only the first two,
        which caused mask-enabled inference to fail before reaching the mask
        head.
        """
        bbox_results, pseudo_boxes = self.simple_test(*args, **kwargs)
        return bbox_results, pseudo_boxes, None
    ...
    def simple_test_bboxes(self,
        ...
        # if stage == 0:
        #     self.Test_P2B_iou.forward(proposals, gt_bboxes)

        if self.with_atten:
            x = self.atten_pts_and_ftmap(x, gt_bboxes, img_metas)

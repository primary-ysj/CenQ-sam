        ...
        if self.debug:
            for i, img_meta in enumerate(img_metas):
                ...
                TestCPRHead.get(self).test_refine_point(refine_pts_d, None, chosen_pts_d,
                                      gt_r_points_d, gt_labels_d, img_metas_d, gt_true_bboxes_d, not_refine_d, refine_scores_d)
...
class EcplHead(AnchorFreeHead):
    ...
    def loss(self, cls_feats, ins_feats, pts_outs, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore=None,gt_true_bboxes=None,
             gt_weights=None):
        ...
        refined_points, refined_scores, not_refine,merge_valid = self.point_refiner(pos_data, neg_data, gt_r_points, gt_labels,
                                                                        img_metas,
                                                                        gt_true_bboxes, not_refine=None)
        ...
        return getattr(self, f'loss{self.loss_type}')(pos_data, neg_data, gt_labels_all, gt_r_points, pseudo_pts,merge_valid,
                                                      gt_true_bboxes,
                                                      gt_weights)
    ...
    def get_targets(self, anchor_pts, pred_pts, valid_flag_list, cls_feats_list, ins_feats_list, gt_points, gt_labels,
                    # label_weights,
        ...
        pos_data_list, neg_data_list = multi_apply(
            ...
            pred_pts, valid_flag_list, cls_feats_list, ins_feats_list,
    ...
    def bag_assign(self, anchor_pts, proposals, cls_feats, ins_feats, gt_points, gt_labels, img_metas,
        ...
        neg_valid_mask = assigned_neg_pts.unsqueeze(0).repeat((self.num_classes, 1))
    ...
    def _get_target_single(self, anchor_pts, pred_pts, valid_flags, cls_feats, ins_feats, gt_points, gt_labels,
                           img_metas,
        ...
        pos_data, neg_data = self.bag_assign(anchor_pts, proposals, cls_pred, ins_pred, gt_points,
                                             gt_labels, img_metas,
                                             gt_bboxes_ignore=None,sample_r=self.sample_r)
...
class TestCPRHead(object):
    ...
    def test_refine_point(self, points, chosen_lvl, chosen_pts_all, gt_r_points, gt_labels, img_metas, gt_true_bboxes,
        ...
        fig = set_plt(plt)
        plt.imshow(img)
        plt.scatter(img_gt_points[:, 0], img_gt_points[:, 1], s=5, c='#22fe61', zorder=11) ## coarse point color 22fe61 ‘g’
        for i in range(len(img_points)):
            ...
            if img_not_refine is None or not img_not_refine[i]:
                ...
                for j in range(len(img_chosen_pts[i])):
                    ...
                    plt.plot([p1[0], p2[0]], [p1[1], p2[1]], ':', color=colors[img_gt_labels[i]]) ##换虚线连接'--', color=colors[img_gt_labels[i]]
                # chosen_pts
                plt.scatter(img_chosen_pts[i][:, 0], img_chosen_pts[i][:, 1], s=5, c='#ed7d31', zorder=10) ## semantic point color:orange ed7d31 'b'
            # chosen fpn level
        ...
        plt.scatter(img_points[is_inside, 0], img_points[is_inside, 1], s=5, c='#ffff00', zorder=10) ## refined point color:yellow, size 'r'
        plt.scatter(img_points[np.logical_not(is_inside), 0], img_points[np.logical_not(is_inside), 1], s=40, c=(0, 0, 0))
        img_name = os.path.split(img_path)[-1]

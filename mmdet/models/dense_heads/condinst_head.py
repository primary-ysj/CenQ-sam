...
class CondInstMaskHead(BaseModule):
    ...
    def training_sample(self, pseudo_boxes, gt_labels, num_levels, others, cascade_weight=None, test=None):
        ...
        if self.merge_featmap and test:
            ...
            x_feat = others['x_feat']
            filtered_scores = others['filtered_scores']
            weight = [self._safe_prune_weight(w) for w in filtered_scores]
    ...
    @staticmethod
    def _safe_prune_weight(scores, eps=1e-6):
        scores = torch.nan_to_num(scores, nan=0.0, posinf=0.0, neginf=0.0).clamp_min(0.0)
        denom = scores.sum(dim=1, keepdim=True)
        if scores.size(1) == 0:
            return scores
        uniform = scores.new_full(scores.shape, 1.0 / scores.size(1))
        return torch.where(denom > eps, scores / denom.clamp_min(eps), uniform)

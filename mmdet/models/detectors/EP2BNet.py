...
class EP2BNet(TwoStageDetector):
    ...
    def forward_train(self,
        ...
        for stage in range(self.num_stages):
            ...
            roi_losses, pseudo_boxes, dynamic_weight = self.roi_head.forward_train(stage, x, img_metas,
                                                                                   ...
                                                                                   generate_proposals,
                                                                                   proposals_valid_list,
                                                                                   neg_proposal_list, neg_weight_list,gt_points,
        ...
        if self.with_rpn:
            ...
            rpn_losses, proposal_list = self.rpn_head.forward_train(
                ...
                pseudo_boxes,
                gt_labels=None,
                ann_weight=dynamic_weight,
                gt_bboxes_ignore=gt_bboxes_ignore,
                proposal_cfg=proposal_cfg)
            losses.update(rpn_losses)

        roi_losses = self.roi_head.forward_train('with_rpn', x, img_metas, None, proposal_list, None, None,
                                                 pseudo_boxes, gt_labels, dynamic_weight,  ## add by fei
                                                 gt_bboxes_ignore, gt_masks,
                                                 **kwargs)
        for key, value in roi_losses.items():
    ...
    def show_imgs(self, cls_scores, level_inds, img_inds, gt_inds, img_metas, gt_labels, img_len, pseudo_boxes,
                  gt_bboxes, feat_map_shape):
        import cv2
        ...
        for i in range(len(img_metas)):
            cls_scores_ = cls_scores[img_inds == i]
            gt_inds_ = gt_inds[img_inds == i]

...
def hbb_proposalbox_aug(proposals_box, num_gen, num_boxes, iou_threshhold, img_meta, if_return_feat=None,
    ...
    # print(bbox_overlaps(gen_bboxes, gt_bboxes_).squeeze(2))
...
def gen_proposals_from_cfg(gt_points, proposal_cfg, img_meta):
    ...
    num_gen = proposal_cfg['gen_num_per_scale']
    base_proposal_list = []
    aug_proposal_list = []
...
class P2Seg(TwoStageDetector):
    ...
    def forward_train(self,
        ...
        if self.with_dense_head:
            ...
            num_level = len(x)
            inputs = (pseudo_boxes_out, gt_labels, num_level, others)
            ...
            a=mask_pred.sigmoid()[1,0]
            cv2.imshow('ims1', np.array(a.detach().cpu()))
            cv2.imshow('gt',igs)
            if cv2.waitKey(0) :
                cv2.destroyAllWindows()

            loss_mask = self.mask_head.loss(img, img_metas, mask_pred, torch.arange(len(mask_pred)), pseudo_boxes_out,
                                            gt_masks, gt_labels, dynamic_weight_out)
    ...
    def simple_test(self, img, img_metas, gt_bboxes, gt_anns_id, gt_true_bboxes, gt_labels,
        ...
        for stage in range(self.num_stages):
            ...
            if stage == 0:
                ...
                generate_proposals, proposals_valid_list = fine_proposals_from_cfg(pseudo_bboxes, fine_proposal_cfg,
                                                                                   img_meta=img_metas, stage=stage)

            det_bboxes, det_labels, pseudo_bboxes ,others= self.roi_head.simple_test_bboxes(x, img_metas, generate_proposals,
                                                                         proposals_valid_list, gt_bboxes, gt_labels,
                                                                         gt_anns_id, stage, rcnn_test_cfg=None,
                                                                         rescale=rescale)
            x = x[1:]
            mask_feat = self.mask_branch(x)
            num_level = len(x)
...
def show(pseudo_box, gt_box, img_meta):
    ...
    for j in range(len(gt_box)):
        ...
        img = cv2.rectangle(img, (int(gt_box[j, 0]), int(gt_box[j, 1])), (int(gt_box[j, 2]), int(gt_box[j, 3])),
                            color=(0, 255, 0))
        img = cv2.rectangle(img, (int(pseudo_box[j, 0]), int(pseudo_box[j, 1])),
                            (int(pseudo_box[j, 2]), int(pseudo_box[j, 3])),
                            color=(0, 255, 255))
        cv2.namedWindow("ims1", 0)
        cv2.resizeWindow("ims1", 640, 480)
        cv2.imshow('ims1', img)

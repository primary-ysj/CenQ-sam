...
class EP2BplusHead(StandardRoIHead):
    ...
    def __init__(self, bbox_roi_extractor, num_stages, bbox_head, bbox_head1=None, stage_modes=['CBP', 'PBR'], top_k=7,
                 ...
                 cluster_mode='classification', with_atten=None,distill_nms=None, **kwargs):
    ...
    def _bbox_forward_train(self, x, proposal_list_base, proposals_list, proposals_valid_list, neg_proposal_list,
        ...
        rois = bbox2roi(proposals_list)
        bbox_results = self._bbox_forward(x, rois, proposal_list_base if proposal_list_base is not None else gt_points,
                                          stage, others=others)
        ...
        # from mmdet.core.bbox.iou_calculators import bbox_overlaps
        # if stage==0:
        #     self.n+=len(torch.cat(gt_true_bboxes))
        #     self.a+=bbox_overlaps(torch.cat(gt_true_bboxes)[:, None], torch.cat(proposals_list_to_merge).reshape(len(torch.cat(gt_true_bboxes)), -1, 4))[:, 0].max(-1)[
        #     0].sum()
        #     print(self.a/self.n)
        ...
        pseudo_boxes, mean_ious, filtered_boxes, filtered_scores, dynamic_weight = merge_box(bbox_results,
                                                                                             ...
                                                                                             topk=self.topk1,
                                                                                             metric_bboxes=gt_true_bboxes)
                                                                                             # flag='distill_nms' if self.Distill_nms and stage==self.num_stages-1  else None)
    ...
    def simple_test_bboxes_pseudo(self,
        ...
        pseudo_boxes, mean_ious, filtered_boxes, filtered_scores, dynamic_weight = merge_box(bbox_results,
                                                                                             ...
                                                                                             gt_bboxes,
                                                                                             img_metas, stage_mode,
                                                                                             topk=self.topk1,flag='distill_nms' if self.Distill_nms and stage==self.num_stages-1 else None)
        if stage==2:
            self.sum_iou+=mean_ious[-1]
            self.sum_num+=1
        ...
        if stage == 0:
            ...
            self.Test_P2B_iou_5(proposals, gt_bboxes, pseudo_boxes)
        ...
        if reg_box is not None:
            ...
            # bbox_results['pseudo_boxes_upper'] = pseudo_boxes_reg.split([len(i) for i in gt_bboxes])
            #
            # # self.Test_P2B_iou_2(bboxes_pred.split([len(i) for i in gt_bboxes]), gt_bboxes, pseudo_boxes)
            # pseudo_boxes_upper = bbox_results['pseudo_boxes_upper']

            bboxes_pred = None
            bboxes_target = None
        else:
            pseudo_boxes_upper=pseudo_boxes
        pseudo_boxes_out = copy.deepcopy(pseudo_boxes)
        # if stage==2:
        #     from mmdet.core.bbox.iou_calculators import bbox_overlaps
        #     gt = torch.cat(gt_bboxes).unsqueeze(1).expand(-1,125,4)
        #     iou = bbox_overlaps(torch.cat(proposals).reshape(-1, 4), gt.reshape(-1, 4), is_aligned=True)
        #     iou = iou.reshape(-1,125)
        #     iou_max, idx = iou.max(dim=1)
        #     pseudo_boxes_reg = torch.cat(proposals).reshape(-1,125,4)[torch.arange(len(torch.cat(proposals).reshape(-1, 125, 4))), idx]
        #     bbox_results['pseudo_boxes_upper'] = pseudo_boxes_reg.split([len(i) for i in gt_bboxes])
        #     pseudo_boxes_upper = bbox_results['pseudo_boxes_upper']
        # else:
        #     pseudo_boxes_upper=pseudo_boxes
        det_bboxes, det_labels = self.pseudobox_to_result(pseudo_boxes, gt_labels, dynamic_weight, gt_anns_id,
                                                          scale_factors, rescale, gt_bboxes, stage)

        bbox_results['others']['dynamic_weight'] = dynamic_weight
    ...
    def pseudobox_to_result(self, pseudo_boxes, gt_labels, dynamic_weight, gt_anns_id, scale_factors, rescale,
        ...
        for i in range(len(pseudo_boxes)):
            ...
            if rescale and boxes.shape[0] > 0:
                ...
                boxes /= scale_factor
                # gt_box/=scale_factor
            iou = bbox_overlaps(boxes, gt_box / scale_factor, is_aligned=True)
            # if stage==2:
            #     self.sum_iou+=iou.sum()
            # self.sum_num+=len(iou)
            # print(self.sum_iou/self.sum_num)
...
class Test_P2B_iou_3(nn.Module):
    def __init__(self, dim_in=256):
        ...
        self.iou1 = torch.zeros(100).to('cuda:0')
        # self.iou2 = torch.zeros(40).to('cuda:0')
        self.iou1_max = torch.zeros(100).to('cuda:0')
        # self.iou2_max = torch.zeros(40).to('cuda:0')
        self.iou1_mean = torch.zeros(100).to('cuda:0')
        # self.iou2_mean = torch.zeros(40).to('cuda:0')
        self.iou_gt_mb = torch.zeros(100).to('cuda:0')

        self.top10_mean = torch.zeros(100).to('cuda:0')
    ...
    def forward(self, pseudo_boxes, gt_bboxes, merge_boxes, mode='best_proposal_iou'):
        ...
        self.iou1 += torch.histc(iou1.reshape(-1), 100, 0, 1)
        # self.iou2 += torch.histc(iou2.reshape(-1), 40, 0, 1)
        self.iou1_max += torch.histc(max_iou1.reshape(-1), 100, 0, 1)
        # self.iou2_max += torch.histc(max_iou2.reshape(-1), 40, 0, 1)
        self.iou1_mean += torch.histc(mean_iou1.reshape(-1), 100, 0, 1)
        # self.iou2_mean += torch.histc(mean_iou2.reshape(-1), 40, 0, 1)
        self.iou_gt_mb += torch.histc(iou_gt_mb[:, 0].reshape(-1), 100, -1, 1)
        self.top10_mean += torch.histc(top10_mean.reshape(-1), 100, 0, 1)

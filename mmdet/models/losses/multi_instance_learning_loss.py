...
class MILLoss(nn.Module):
    ...
    def gfocal_loss(self, p, q, w=1.0):
        # Keep saturated PBR probabilities away from log(0).  Adding eps only
        # inside the logarithm still leaves an O(1 / eps) backward gradient.
        p = torch.nan_to_num(p, nan=0.5, posinf=1.0 - self.eps,
                             neginf=self.eps).clamp(self.eps, 1.0 - self.eps)
        l1 = (p - q) ** 2
        l2 = q * p.log() + (1 - q) * (1 - p).log()
    ...
    def forward(self, bag_cls_prob, bag_ins_outs, labels, valid, weight=None):
        ...
        if self.loss_type == 'gfocal_loss':
            ...
            # method 2
            prob = torch.nan_to_num(prob, nan=0.5, posinf=1.0,
                                    neginf=0.0).clamp(0, 1)
            # modified by fei ##############################################################3

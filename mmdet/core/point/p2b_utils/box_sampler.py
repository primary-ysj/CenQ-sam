...
def CBP_proposals_from_cfg(gt_points, proposal_cfg, img_meta):
    ...
    ori_scale = proposal_cfg['ori_scale'] if 'ori_scale' in proposal_cfg else 100
    ...
    for i in range(len(gt_points)):
        ...
        base = min(img_w, img_h) / ori_scale
        base_proposals = []
...
def gen_negative_proposals(gt_points, proposal_cfg, aug_generate_proposals, img_meta):
    ...
    if 'add_small_neg' in proposal_cfg:
        with_small_negs = proposal_cfg['add_small_neg']
    else:
        with_small_negs=False
    ori_scale = proposal_cfg['ori_scale'] if 'ori_scale' in proposal_cfg else 100
    ...
    for i in range(len(gt_points)):
        ...
        if with_small_negs:
            img_h, img_w, _ = img_meta[i]['img_shape']
            base = min(img_w, img_h) / ori_scale
            x1 =  torch.rand(num_neg_gen) * w
            y1 =  torch.rand(num_neg_gen) *h
            x2 = x1 + torch.rand(num_neg_gen) * torch.clamp( w - x1,0,base*8)
            y2 = y1 + torch.rand(num_neg_gen) * torch.clamp(h - y1,0,base*8)
            neg_bboxes_small = torch.stack([x1, y1, x2, y2], dim=1).to(gt_points[0].device)
            neg_bboxes=torch.cat([neg_bboxes,neg_bboxes_small])

        # if gt_points[i].shape[-1]==4:
        #     iou = bbox_overlaps(neg_bboxes, gt_points[i])
        # else:
...
def random_shake_sample(proposal_boxes, rate):
    shake_pps_list = []
    batch = [len(p) for p in proposal_boxes]
    rate = torch.split(rate, batch)
    for i in range(len(proposal_boxes)):
        pps = proposal_boxes[i]
        w, h = bbox_xyxy_to_cxcywh(pps)[:, 2], bbox_xyxy_to_cxcywh(pps)[:, 3]
        r = 1 - rate[i]
        x1_offset = r * w / 2 * (torch.rand(batch[i], device=r.device) * 2 - 1)
        y1_offset = r * h / 2 * (torch.rand(batch[i], device=r.device) * 2 - 1)
        x2_offset = r * w / 2 * (torch.rand(batch[i], device=r.device) * 2 - 1)
        y2_offset = r * h / 2 * (torch.rand(batch[i], device=r.device) * 2 - 1)
        offset = torch.stack([x1_offset, y1_offset, x2_offset, y2_offset], dim=1)
        shake_pps = pps + offset
        shake_pps_list.append(shake_pps)

    return shake_pps_list


def random_scale_sample(proposal_boxes, rate):
    scale_pps_list = []
    batch = [len(p) for p in proposal_boxes]
    rate = torch.split(rate, batch)
    for i in range(len(proposal_boxes)):
        pps = proposal_boxes[i]
        pps_xywh = bbox_xyxy_to_cxcywh(pps)
        offset = ((torch.rand((batch[i], 2), device=pps_xywh.device)) * 1.5 + 0.5)
        offset = (1 - rate[i])[:, None] * (offset - 1) + 1

        pps_xywh[:, 2:] = pps_xywh[:, 2:] * offset
        scale_pps = bbox_cxcywh_to_xyxy(pps_xywh)
        scale_pps_list.append(scale_pps)

    return scale_pps_list
